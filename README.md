...
- **Global recording, no object to pass around.** `some_crate::metrics().foo.inc()`
...
```rust
// Declare — expands to a real struct with autocomplete-able fields.
embeprom::metrics! {
...
// call to `metrics()` self-registers the group; no setup step needed.
...
When a registry is directly visible to a declaration,
`metrics! { registry = REGISTRY; ... }` targets it without installation.
`metrics! { registration = manual; ... }` disables accessor registration so
the same static group can be registered explicitly with multiple registries.
